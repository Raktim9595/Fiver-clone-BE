...
import com.raktim.fiverclone.seeds.experienceLevel.ExperienceLevelController;
...
public class ExperienceLevelControllerTest {
    ...
    @Test
    @DisplayName("""
            When called GET /api/experienceLevel When called,
            Then it should hit the respective service method and return proper data
            """)
    public void test_getExperienceLevels() throws Exception {
        mockMvc.perform(get("/api/experienceLevel")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());

        verify(experienceLevelService, times(1)).getExperienceLevels();
    }
}
